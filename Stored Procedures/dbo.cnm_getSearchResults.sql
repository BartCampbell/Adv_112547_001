...
BEGIN
	...
	BEGIN
		...
		SELECT PO.ProviderOffice_PK,
				ROW_NUMBER() OVER(
				ORDER BY 
					CASE WHEN @Order='ASC'  THEN CASE @SORT WHEN 'CLI' THEN PO.LocationID WHEN 'AL1' THEN PO.Address WHEN 'AL2' THEN ZC.City WHEN 'PN' THEN PO.ContactNumber WHEN 'FN' THEN PO.FaxNumber WHEN 'CP' THEN PO.ContactPerson WHEN 'Pr' THEN CASE WHEN COUNT(DISTINCT C.Channel_PK)=1 THEN MIN(C.Channel_Name) ELSE 'Multiple' END ELSE NULL END END ASC,
					CASE WHEN @Order='DESC' THEN CASE @SORT WHEN 'CLI' THEN PO.LocationID WHEN 'AL1' THEN PO.Address WHEN 'AL2' THEN ZC.City WHEN 'PN' THEN PO.ContactNumber WHEN 'FN' THEN PO.FaxNumber WHEN 'CP' THEN PO.ContactPerson WHEN 'Pr' THEN CASE WHEN COUNT(DISTINCT C.Channel_PK)=1 THEN MIN(C.Channel_Name) ELSE 'Multiple' END ELSE NULL END END DESC,
				...
				PO.LocationID,PO.Address,ZC.City,ZC.County,ZC.State,ZC.Zipcode,PO.ContactNumber,PO.FaxNumber,PO.ContactPerson
